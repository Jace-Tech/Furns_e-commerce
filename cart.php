                                    ...
                                    <?php foreach(getCurrentProduct($product_id) as $products): extract($products); ?>  
                                        <tr>
                                            <td class="product-thumbnail">
                                                <a href="#"><img class="img-responsive ml-3" src="./admin/images/products/<?= json_decode($image)[0]; ?>" alt="" /></a>
                                            </td>
                                            <td class="product-name"><a href="#"><?= $product_name; ?></a></td>
                                            <td class="product-price-cart"><span class="amount">&#8358; <?= number_format($new_price); ?></span></td>
                                            <td class="product-quantity">
                                                <div class="cart-plus-minus">
                                                    <input class="cart-plus-minus-box" type="text" name="qtybutton" value="<?= $quantity; ?>" />
                                                </div>
                                            </td>
                                            <td class="product-subtotal">&#8358;<?= number_format(intval($quantity) * intval($new_price)); ?></td>
                                            <td class="product-remove">
                                                <!-- <a href="#"><i class="icon-pencil"></i></a> -->
                                                <a href="./assets/handlers/cart.php?del_id=<?= $cart_id; ?>"><i class="icon-close"></i></a>
                                            </td>
                                        </tr>
                                    <?php endforeach; ?>  
                                <?php endforeach; ?>
